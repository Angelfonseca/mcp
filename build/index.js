...
server.tool("search-with-serper", "Tool to search the web with Serper", {
...
server.tool("fetch-url", "Tool to fetch safely a URL", {
...
// MySQL Tools
server.tool("mysql-analyze", "Analiza completamente una base de datos MySQL", {
...
server.tool("mysql-query", "Ejecuta una query personalizada en MySQL", {
    host: z.string().describe("Host del servidor MySQL"),
    port: z.number().optional().describe("Puerto del servidor MySQL (default: 3306)"),
    username: z.string().describe("Usuario de MySQL"),
    password: z.string().describe("Contraseña de MySQL"),
...
server.tool("mysql-search", "Busca datos específicos en MySQL usando texto libre", {
    host: z.string().describe("Host del servidor MySQL"),
    port: z.number().optional().describe("Puerto del servidor MySQL (default: 3306)"),
    username: z.string().describe("Usuario de MySQL"),
    password: z.string().describe("Contraseña de MySQL"),
...
server.tool("mysql-embeddings", "Genera embeddings para datos de MySQL", {
    host: z.string().describe("Host del servidor MySQL"),
    port: z.number().optional().describe("Puerto del servidor MySQL (default: 3306)"),
    username: z.string().describe("Usuario de MySQL"),
    password: z.string().describe("Contraseña de MySQL"),
...
// MongoDB Tools
server.tool("mongodb-analyze", "Analiza completamente una base de datos MongoDB", {
...
server.tool("mongodb-query", "Ejecuta una query personalizada en MongoDB", {
    connectionString: z.string().optional().describe("String de conexión MongoDB"),
    host: z.string().optional().describe("Host del servidor MongoDB"),
    port: z.number().optional().describe("Puerto del servidor MongoDB (default: 27017)"),
    username: z.string().optional().describe("Usuario de MongoDB"),
...
server.tool("mongodb-search", "Busca datos específicos en MongoDB usando texto libre", {
    connectionString: z.string().optional().describe("String de conexión MongoDB"),
    host: z.string().optional().describe("Host del servidor MongoDB"),
    port: z.number().optional().describe("Puerto del servidor MongoDB (default: 27017)"),
    username: z.string().optional().describe("Usuario de MongoDB"),
...
server.tool("mongodb-embeddings", "Genera embeddings para datos de MongoDB", {
    connectionString: z.string().optional().describe("String de conexión MongoDB"),
    host: z.string().optional().describe("Host del servidor MongoDB"),
    port: z.number().optional().describe("Puerto del servidor MongoDB (default: 27017)"),
    username: z.string().optional().describe("Usuario de MongoDB"),
...
// PostgreSQL Tools
server.tool("postgresql-analyze", "Analiza completamente una base de datos PostgreSQL", {
...
server.tool("postgresql-query", "Ejecuta una query personalizada en PostgreSQL", {
    host: z.string().describe("Host del servidor PostgreSQL"),
    port: z.number().optional().describe("Puerto del servidor PostgreSQL (default: 5432)"),
    username: z.string().describe("Usuario de PostgreSQL"),
    password: z.string().describe("Contraseña de PostgreSQL"),
...
server.tool("postgresql-search", "Busca datos específicos en PostgreSQL usando texto libre", {
    host: z.string().describe("Host del servidor PostgreSQL"),
    port: z.number().optional().describe("Puerto del servidor PostgreSQL (default: 5432)"),
    username: z.string().describe("Usuario de PostgreSQL"),
    password: z.string().describe("Contraseña de PostgreSQL"),
...
server.tool("postgresql-embeddings", "Genera embeddings para datos de PostgreSQL", {
    host: z.string().describe("Host del servidor PostgreSQL"),
    port: z.number().optional().describe("Puerto del servidor PostgreSQL (default: 5432)"),
    username: z.string().describe("Usuario de PostgreSQL"),
    password: z.string().describe("Contraseña de PostgreSQL"),
...
server.tool("postgresql-similarity-search", "Busca datos usando similitud de embeddings en PostgreSQL", {
    host: z.string().describe("Host del servidor PostgreSQL"),
    port: z.number().optional().describe("Puerto del servidor PostgreSQL (default: 5432)"),
    username: z.string().describe("Usuario de PostgreSQL"),
    password: z.string().describe("Contraseña de PostgreSQL"),
...
server.tool("generate-data-report", "Crea reportes ejecutivos profesionales con análisis estadístico completo, visualizaciones, insights y recomendaciones. Exporta a PDF de alta calidad, DOCX para editar, o HTML interactivo. Incluye resumen ejecutivo, estadísticas descriptivas, correlaciones, outliers y conclusiones automáticas.", {
...
server.tool("analyze-dataframe", "Motor de análisis estadístico avanzado para datasets grandes: estadísticas descriptivas completas, matrices de correlación, detección de outliers automática, clustering K-means con PCA, y análisis de segmentación. Optimizado para datasets de millones de registros con muestreo inteligente y control de memoria.", {
    ...
    try {
        ...
        switch (analysisType) {
            ...
            case 'anomalies':
                if (!anomalyColumn)
                    throw new Error('Se requiere anomalyColumn para análisis de anomalías');
            ...
            case 'clustering':
                ...
                const clusteringResult = reportsHelpers.performKMeansClustering(clusterData, clusterColumns[0], clusterColumns[1], clusterCount);
        ...
        if (groupBy && aggregation && targetColumn) {
            ...
            result.analysis.groupedAnalysis = await calculateGroupedAnalysis(analysisData, groupBy, aggregation, targetColumn);
        }
 ...
 */
async function calculateOptimizedStats(data) {
    const result = {};
    if (data.length === 0)
        return result;
    const numericColumns = Object.keys(data[0]).filter(col => data.some(row => typeof row[col] === 'number' && !isNaN(row[col])));
        ...
        columnBatch.forEach(col => {
            ...
            if (values.length > 0) {
                ...
                batchResults[col] = {
                    ...
                    std: parseFloat(Math.sqrt(values.map((x) => Math.pow(x - mean, 2))
                        .reduce((a, b) => a + b, 0) / values.length).toFixed(4)),
 ...
 */
async function calculateGroupedAnalysis(data, groupBy, aggregation, targetColumn) {
    const grouped = {};
    // Agrupar datos
    data.forEach(row => {
        ...
        if (values.length > 0) {
            switch (aggregation) {
                ...
                case 'sum':
                    ...
                    break;
                case 'avg':
                    result[key] = parseFloat((values.reduce((a, b) => a + b, 0) / values.length).toFixed(4));
                    break;
            }
        }
    });
...
server.tool("upload-dataframe-to-database", "Sube un array de objetos a la base de datos seleccionada, con opción de generar embeddings", {
    target: z.enum(['mysql', 'postgresql', 'mongodb']).describe("Tipo de base de datos destino"),
    connection: z.any().describe("Configuración de conexión"),
    table: z.string().optional().describe("Nombre de tabla (MySQL/PostgreSQL)"),
    collection: z.string().optional().describe("Nombre de colección (MongoDB)"),
    ...
    try {
        if (embed) {
            if (!textField)
                throw new Error('Debe especificar textField para generar embeddings');
            await generateEmbeddingsForData(data, textField, embeddingField);
        }
        switch (target) {
            case 'mysql': {
                if (!table)
                    throw new Error('Debe proporcionar el nombre de la tabla');
            ...
            case 'mongodb': {
                if (!collection)
                    throw new Error('Debe proporcionar el nombre de la colección');
    ...
    while (attempt < retries) {
        try {
            ...
            switch (source.type) {
                case 'csv':
                    if (!source.filePath)
                        throw new Error('Se requiere filePath para CSV');
                ...
                case 'excel':
                    if (!source.filePath)
                        throw new Error('Se requiere filePath para Excel');
                ...
                case 'api':
                    if (!source.url)
                        throw new Error('Se requiere URL para API');
 ...
 */
async function insertDataInBatches(data, target) {
 ...
 */
async function insertBatch(batch, dbType, connection, table, collection) {
    if (batch.length === 0)
        return;
    switch (dbType) {
        case 'mysql': {
            if (!table)
                throw new Error('Tabla requerida para MySQL');
            const conn = await mysqlHelpers.createMySQLConnection(connection);
            try {
        ...
        case 'postgresql': {
            if (!table)
                throw new Error('Tabla requerida para PostgreSQL');
            const pool = postgresHelpers.createPostgreSQLPool(connection);
            try {
                const columns = Object.keys(batch[0]);
                for (const row of batch) {
        ...
        case 'mongodb': {
            if (!collection)
                throw new Error('Colección requerida para MongoDB');
            const { client, db } = await mongoHelpers.createMongoDBConnection(connection);
            try {
                const coll = db.collection(collection);
...
server.tool("monitor-performance", "Monitorea el rendimiento del sistema y proporciona recomendaciones para optimizar el procesamiento de grandes datasets", {
 ...
 */
function calculateSystemScore(cpuTime, memTime, ioTime) {
    const score = (cpuTime * 0.4) + (memTime * 0.4) + (ioTime * 0.2);
    if (score < 50)
        return `${score.toFixed(0)} (Excelente)`;
    if (score < 100)
        return `${score.toFixed(0)} (Bueno)`;
    if (score < 200)
        return `${score.toFixed(0)} (Regular)`;
    return `${score.toFixed(0)} (Lento)`;
}
/**
 * Genera recomendaciones específicas para procesamiento de datos
 */
function generateDataProcessingRecommendations(dataSize, estimatedMemoryMB, availableMemoryMB) {
 ...
 */
function generateSuggestedParameters(dataSize, estimatedMemoryMB, availableMemoryMB) {
    const memoryRatio = estimatedMemoryMB / availableMemoryMB;
...
server.tool("list-analysis-templates", "Muestra catálogo completo de templates de análisis Python disponibles: exploración de datos, machine learning, clustering, series de tiempo y estadísticas. Incluye descripción detallada, parámetros requeridos, columnas necesarias y ejemplos de uso para cada template.", {}, async () => {
    try {
...
server.tool("python-quick-stats", "Genera estadísticas rápidas y visualizaciones usando Python para un dataset", {
...
server.tool("load-data-source", "Carga y convierte archivos de datos a formato tabular: CSV (auto-detecta encoding/delimitadores), Excel (.xlsx/.xls), JSON y URLs de APIs REST. Maneja archivos locales, URLs file:// y APIs HTTP. Detecta automáticamente tipos de datos y convierte a array de objetos JavaScript listos para análisis.", {
...
server.tool("create-database-from-data", "Analiza datos tabulares (CSV/JSON) y genera automáticamente: esquemas SQL completos, diagramas entidad-relación, documentación técnica y archivos .sql listos para ejecutar en MySQL/PostgreSQL/SQLite. Detecta tipos de datos, claves primarias y relaciones automáticamente.", {
...
server.tool("generate-advanced-sql-queries", "Genera consultas SQL especializadas para análisis de datos: estadísticas (MIN/MAX/AVG), agrupaciones (GROUP BY), análisis temporal por fechas, rankings, detección de outliers, consultas analíticas y reportes ejecutivos. Optimizado para MySQL/PostgreSQL/SQLite.", {
...
function generateBasicQueries(tableName, columns) {
...
function generateStatisticalQueries(tableName, numericColumns, dbType) {
...
function generateGroupingQueries(tableName, columns, numericColumns) {
...
function generateTemporalQueries(tableName, dateColumns, numericColumns, dbType) {
...
function generateAdvancedQueries(tableName, columns, numericColumns, dbType) {
...
function generateAnalyticsQueries(tableName, numericColumns, dbType) {
...
function generateReportingQueries(tableName, columns, numericColumns, dateColumns, dbType) {
...
function generateCustomQueries(tableName, requirements, columns, dbType) {
