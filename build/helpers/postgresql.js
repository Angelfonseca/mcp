 ...
 */
function createPostgreSQLPool(config) {
    return new Pool({
        host: config.host,
 ...
 */
async function analyzePostgreSQLDatabase(config) {
 ...
 */
async function executePostgreSQLQuery(config, query, params) {
    const pool = createPostgreSQLPool(config);
 ...
 */
async function searchPostgreSQLData(config, searchTerm, tables) {
    const pool = createPostgreSQLPool(config);
    try {
        const results = [];
 ...
 */
async function generatePostgreSQLEmbeddings(config, tableName, textColumn, embeddingApiUrl = 'http://localhost:11434/api/embeddings') {
    const pool = createPostgreSQLPool(config);
    try {
 ...
 */
async function searchPostgreSQLBySimilarity(config, tableName, embeddingColumn, queryEmbedding, limit = 10) {
    const pool = createPostgreSQLPool(config);
    try {
...
export { analyzePostgreSQLDatabase, executePostgreSQLQuery, searchPostgreSQLData, generatePostgreSQLEmbeddings, searchPostgreSQLBySimilarity, createPostgreSQLPool, };
