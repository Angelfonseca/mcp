...
export class SQLGenerator {
     ...
     */
    generateQueries(data, tableName, options) {
     ...
     */
    saveMermaidDiagram(content, filename) {
    ...
    generateDatabaseCreation(dbName, dbType) {
    ...
    generateTableSQL(table, options) {
    ...
    generateIndexesSQL(table, dbType) {
        let sql = '';
        if (table.indexes) {
    ...
    generateForeignKeysSQL(schema, dbType) {
     ...
     */
    analyzeDataAndGenerateSchema(data, tableName) {
        if (data.length === 0) {
    ...
    inferColumnType(columnName, data) {
        ...
        // Detectar booleanos
        const isAllBooleans = values.every(v => v === true || v === false || v === 'true' || v === 'false' || v === 0 || v === 1);
        if (isAllBooleans) {
            return 'BOOLEAN';
        }
        // Texto
