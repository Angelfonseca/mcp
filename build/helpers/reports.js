 ...
 */
function generateSimpleChart(config) {
    const { type, title, data, width = 800, height = 400 } = config;
    if (data.length === 0)
        return '<p>No hay datos para el gráfico</p>';
 ...
 */
export async function exportToPDF(htmlContent, outputPath) {
 ...
 */
export async function exportToDOCX(report, outputPath) {
 ...
 */
export function detectAnomalies(data, column) {
    const values = data
        .map(row => row[column])
 ...
 */
export function performKMeansClustering(data, xColumn, yColumn, k = 3) {
    const points = data
    ...
    const clusters = centroids.map((centroid, index) => ({
        id: index,
        centroid,
        points: points.filter((point) => point.cluster === index).map(p => p.original),
        size: points.filter((point) => point.cluster === index).length
    }));
    const insights = [
 ...
 */
async function generateReport(template, options) {
    const { format, outputPath, includeCharts = true, theme = 'light' } = options;
    // Preparar datos para las secciones
    const processedSections = await Promise.all(template.sections.map(async (section) => {
        const processedSection = { ...section };
        if (section.type === 'table' && section.data) {
            processedSection.tableHtml = generateTable(section.data);
        }
        if (section.type === 'chart' && section.chartConfig && includeCharts) {
            processedSection.chartHtml = generateSimpleChart(section.chartConfig);
        }
    ...
    if (options.format === 'markdown') {
        ...
        if (template.author)
            markdown += `**Autor:** ${template.author}\n\n`;
        if (template.date)
            markdown += `**Fecha:** ${template.date}\n\n`;
...
export { generateReport, generateSimpleChart, generateTable, generateDataInsights, };
