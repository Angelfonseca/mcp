...
export interface PythonResult {
  ...
  plots?: string[]; // Rutas a archivos de gráficos generados
}
 ...
 */
function generatePythonCode(userCode: string, dataFile: string | null, includePlots: boolean, outputDir: string): string {
  return `
import sys
import json
import pandas as pd
 ...
 */
function runPythonScript(scriptPath: string, timeout: number): Promise<{ success: boolean; output: string; error?: string }> {
 ...
 */
async function installPythonPackages(packages: string[]): Promise<void> {
 ...
 */
export const ANALYSIS_TEMPLATES: { [key: string]: AnalysisTemplate } = {
  'data_exploration': {
 ...
 */
export function applyAnalysisTemplate(templateName: string, parameters: { [key: string]: any } = {}): string {
