...
interface RequestOptions extends RequestInit {
  /** Tiempo máximo de espera antes de abortar la petición (ms). */
  timeoutMs?: number;
  /** Indica si se debe intentar parsear la respuesta como JSON. */
  expectJson?: boolean;
}
 ...
 */
async function safeFetch<T = unknown>(url: string, options: RequestOptions = {}): Promise<T> {
  const {
    timeoutMs = 10_000,
    expectJson = true,
...
async function searchWithSerper<T = unknown>(query: string): Promise<T> {
  if (!SERPER_API_KEY) {
  ...
  return safeFetch<T>(SERPER_API_URL, {
    method: "POST",
