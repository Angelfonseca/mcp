 ...
 */
function mapEncoding(detectedEncoding: string | null): BufferEncoding {
  if (!detectedEncoding) return 'utf8';
 ...
 */
export async function readCSV(filePath: string, options: {
  batchSize?: number;
  maxRows?: number;
  encoding?: string;
  delimiter?: string;
  ...
  let detectedEncoding: BufferEncoding;
  
  if (encoding) {
    detectedEncoding = encoding as BufferEncoding;
  } else {
 ...
 */
export function readExcel(filePath: string, options: {
  sheetName?: string;
  maxRows?: number;
  batchSize?: number;
} = {}): any[] {
  ...
  try {
    ...
    const data = (jsonData.slice(1, maxRows + 1) as any[][]).map((row: any[], index) => {
